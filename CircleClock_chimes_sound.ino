/* Simple Hr/Min clock with auto intensity
   ...
   obtrusive.  For silent operation, simply unplug the sound generator unit! 
...
void loop() {
  ...
  if ( ( (mins == 0) || (mins == 30)) && (sec < 20) ) {
    if ((mins == 0) & (sec == 0)) {
      ...
      for (int n = 0; n < m; n++) {
        if (night == 1) { tone(soundPin, 220, nighttone); }
        else { tone(soundPin, 220, daytone); }
        delay(400);
      }      
    }
